/**
 * @file pulseg_core.c
 * @brief Collection lifecycle, consistency checks and the segment remap.
 *
 * Every allocation made anywhere in the pipeline is released by
 * pulseg_sequence_descriptor_free() or pulseg_collection_free(), and the
 * cross-subsequence checks a collection must satisfy before any consumer sees
 * it are here too, so a scanner build that only loads a cache links this file
 * without the conversion passes.
 */
...
void pulseg_sequence_descriptor_free(pulseg_sequence_descriptor *d)
{
    ...
    if (d->grad_shape_energy)
    {
        PULSEG_FREE(d->grad_shape_energy);
        d->grad_shape_energy = NULL;
    }
...
/* Public wrapper around pulseg__check_consistency.
 *
...
int pulseg_check_consistency(const pulseg_collection *coll, pulseg_diagnostic *diag)
{
    ...
    return pulseg__check_consistency(
        (pulseg_collection *)coll, /*allow_variable_rf=*/1, diag);
}
